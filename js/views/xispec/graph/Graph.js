 ...
 * @param {AnnotatedSpectrumModel} model - The data backbone-models containing spectrum information
 ...
 * @property {AnnotatedSpectrumModel} model - Reference to the data backbone-models
...
/**
 * Initializes the graph with spectrum data from the backbone-models.
...
Graph.prototype.setData = function () {
    ...
    // 	this.margin.bottom += (this.backbone-models.isLinear) ? 20 : 45;
...
Graph.prototype.updatePeakColors = function () {
    ...
    if (model.highlights.length === 0 || !this.model.get("hideNotSelectedFragments")) {
        ...
        // let pepFragVis = backbone-models.get('pepFragmentVis');
        // if (pepFragVis === 'pep1'){
        // 	let p2FragPeaks = this.fragment_peaks.filter(function(p){
        // 		return p.fragments.filter(function(f){ return f.peptideId === 1}).length > 0;
        // 	})
        // 	p2FragPeaks.forEach(function(p){ p.line.attr("stroke", backbone-models.get('peakColor')); });
        // }
        ...
        // 	})
        // 	p1FragPeaks.forEach(function(p){ p.line.attr("stroke", backbone-models.get('peakColor')); });
        // }
