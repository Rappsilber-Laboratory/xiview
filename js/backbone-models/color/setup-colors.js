/**
 * @fileoverview Color backbone-models initialization and factory functions for xiVIEW.
 * Sets up all predefined color schemes (default, group, inter-protein, distance, score) for crosslinks and proteins.
 * Includes factory function to dynamically create color clms-backbone-models from metadata (numeric → threshold, hex → ordinal with hex values, categorical → ordinal).
 * Manages color backbone-models collections with event listeners for metadata updates.
 */
import $ from "jquery";
...
/**
 * Initializes all color clms-backbone-models for crosslinks and proteins.
...
export const setupColourModels = function (compositeModelInst, userConfig) {
    ...
    /**
     * Factory function for creating group color backbone-models.
     * Gets search map from global clmsModel instance.
     * @returns {GroupColourModel} New group color backbone-models instance
     */
    ...
    //init highest score colour backbone-models
    const clmsModel = compositeModelInst.get("clmsModel"); //todo - shouldn't have this static reference to backbone-models here
...
/**
 * Factory function that creates appropriate color backbone-models based on metadata field analysis.
 * Analyzes all values for the field and determines best color backbone-models type:
 ...
 * @param {string} label - Display label for the color backbone-models
 * @param {Map} objs - Map of objects (crosslinks or proteins) with getMeta() method
 * @returns {ColourModel} Appropriate color backbone-models subclass instance
 */
