...
function makeSVGDoc(svgElem) {
    ...
    pruneInvisibleSubtrees(cloneSVG, svgElem);
    ...
    const addDummy = function (dummySVGElem, cloneSVG, origSVG, transferAttr) {
        ...
        Object.keys(transferAttr).forEach(function (attr) {
            const val = cloneSVG.getAttribute(attr) || cloneSVG.style [attr] || getComputedStyleCssText(origSVG, attr);
            if (val != null) {
    ...
    if (parentAdded) {
        cloneSVG.setAttribute("style", getComputedStyleCssText(svgElem));
    }
...
function getComputedStyleCssText(element, field) {
...
function pruneInvisibleSubtrees(clonedElement, matchingOriginalElement) {
    if (doPruneInvisible) {
        ...
        pruneConditionSets.forEach(function (conditionSet) {
            if (!prune) {
        ...
        if (prune && clonedElement.parentNode) {
            ...
            if (clonedChildren && clonedChildren.length) {
                ...
                for (let i = clonedChildren.length; --i >= 0;) {
                    pruneInvisibleSubtrees(clonedChildren[i], matchingOriginalChildren[i]);
                }
            }
        }
    }
...
function saveSVGDocs(svgDocs) {
    ...
    svgDocs.forEach(function (svgDoc, i) {
        const xmlStr = makeXMLStr(xmls, svgDoc);
